...
本篇文章是在 `qinghub.top` 站点在使用 Markdown 写作方面的说明以及常用功能代码模块。
本站基于开源项目 Hugo 搭建，在此之上进行了功能修改，以下为部分功能修改和添加的引用说明。


## 🔐 文章访问密码

设置打开文章需要输入密码才可查看的代码，代码弹窗样式已经配置在本站源代码后台，使用前需要将以下代码引用块放在需要设置密码的文章正文最前面，标题的后面。
...
需要注意的是：由于本站为静态网站，此密码设置只是在前端，通过技术手段可能会被破解，重要涉密资料不建议使用此方法设置加密！
...
本站 GitHub 后台已经配置了下载附件模块的样式，如果需要添加下载附件模块，直接在文章内引用代码块即可。

- 在 `title=` 引号内可以输入需要下载的文件名称。
- 在 `sub=` 引号内可以输入提示文字。
- 在 `url=` 引号内输入你的下载链接
- 在 `pass=` 引号内输入你的下载密码
如果不需要密码，可以将 `pass="你的下载密码"` 列删除
...
由于 Markdown 写作样式代码过多，这里暂时不展示。后续如果有需要，我会随时添加内容在此文章下。
